...
void HAL_ESP32::SetOutputState(uint8_t outputId, RelayState state)
{
    ...
    if (outputId <= 6)
    {
        ...
        ESP_LOGD(TAG, "0 PCF8574A reply PCF8574A_Value = %i", PCF8574A_Value);
...
void HAL_ESP32::ConfigureI2C(void (*TCA6408Interrupt)(void), void (*TCA9534AInterrupt)(void))
{
    ...
    uint8_t data_wr[8] = {0};
    data_wr[0] = Uint8ToBcd(30); // seconds 0 - 60
    data_wr[1] = Uint8ToBcd(47); // minutes 0 - 60
    data_wr[2] = Uint8ToBcd(21); // hours values 00 - 23;    set 12h bit 6 with b0100 0000 = 0x40  | DS3231_12HOUR_FLAG
    data_wr[3] = Uint8ToBcd(1);  // weekday 1 - 7;
    data_wr[4] = Uint8ToBcd(15); // mday 0 - 31
    data_wr[5] = Uint8ToBcd(04); // month 01 - 12;  bit 7 is century bit
    data_wr[6] = Uint8ToBcd(41); // 00 - 99; if century rolls over set century flag
    // calculate the century flag
    uint8_t year = 2036 - 2000; // actual year to set minus 2000
    ...
    data_wr[5] = (Uint8ToBcd(03) | centuryFlag); // month 01 - 12;  bit 7 is century bit
    data_wr[6] = Uint8ToBcd(36);                 // 00 - 99; if century rolls over set century flag
    ...
    uint8_t wday = BcdToUint8(data_rd[3] & 0x07); // warum  - 1 ?;
    ...
    if (ESP_OK == test4i2cDevice(I2C_NUM_0, PCF8574A_ADDRESS))
    {
        ...
        else
        {
            ...
            attachInterrupt(PCF8574A_INTERRUPT_PIN, TCA9534AInterrupt, FALLING);
