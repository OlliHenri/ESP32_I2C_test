...
#define PCF8574B_INTERRUPT_PIN GPIO_NUM_35
...
private:
    ...
    i2c_cmd_handle_t beginI2C(i2c_port_t i2c_num, uint8_t deviceAddress);
    // i2c send data ; i2c: Writes a single byte to a slave devices
    esp_err_t writeByteI2C(i2c_cmd_handle_t cmd, uint8_t data);
    // i2c read data; reads a singel byte
    uint8_t readByteI2C(i2c_cmd_handle_t cmd, uint8_t dev);
    // i2c end transmission ;  terminates i2c transmission
    esp_err_t endTransmissionI2C(i2c_port_t i2c_num, i2c_cmd_handle_t cmd);
    ...
    esp_err_t test4DS3231();

    /*     uint8_t getRegI2C(uint8_t regAddress);
